    ...
    <div class="py-12 max-w-3xl mx-auto sm:px-6 lg:px-8">
        <form method="POST" action="{{ route('admin.assignments.store') }}" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
            ...
            <div>
                ...
                <x-text-input id="title" name="title" :value="old('title', $assignment->title)" type="text" class="mt-1 block w-full" required />
                <x-input-error :messages="$errors->get('title')" class="mt-2" />
            </div>

            <div class="mt-4">
                <x-input-label for="description" :value="__('Description')" />
                <textarea id="description" name="description" :value="old('description', $assignment->description)" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"></textarea>
